// Tests for scheduler interval parsing and a call with the overlap guard set.
// These do not exercise interval ticking or prove that overlapping work is skipped.
...
describe("#59 runScheduledCleanup overlap guard", () => {
  test("skips when a previous cycle is still running — never overlaps", async () => {
    ...
    // This checks only that the call resolves with the guard set. Caught
    // plan/execute failures also resolve, so it does not prove work was skipped.
